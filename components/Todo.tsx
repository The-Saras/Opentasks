...
import TaskCard from "./TaskPoster";
...
export const Todo = () => {
  ...
  useEffect(() => {
    if (data) {
      ...
      const pending = data.getuserTodos.filter(
        (todo: Todo) => todo.completed === "PENDING"
      );
      const working = data.getuserTodos.filter(
        (todo: Todo) => todo.completed === "WORKING"
      );
      const done = data.getuserTodos.filter(
        (todo: Todo) => todo.completed === "DONE"
      );
